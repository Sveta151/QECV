...
directory = '../final/q2_document_retrieval'
...
merged_df.to_csv(directory + '/q2_merged_results.csv', index=False)

df_filtered = merged_df[merged_df['document_weight_question_2'] != 'unweighted']
...
for i in merged_df.index:
    print(i)
    question = merged_df['generated_question_2'][i]
    document = merged_df['document_question_2'][i]
    summary = generate_summary(document, question)
    results.append(summary)

merged_df['summary_2'] = results

merged_df.to_csv('../final/q2_with_summaries.csv', index = False)